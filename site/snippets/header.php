<div class="header" role="banner">
	<div class="beamBox">
		<div class="beam "></div>
	</div>
	<div class="arrowBox">
		<div class="headerPillarAndIcon">
			<div class="headerArrowContainer">
				<svg class="headerArrow" height="40%" version="1.1" viewbox="0 0 28 28" width="50%" xmlns="http://www.w3.org/2000/svg">
				<defs>
					<polygon id="path-1" points="2 -3.55271368e-15 26 -3.55271368e-15 14 28" ></polygon>
					<mask fill="white" height="28" id="mask-2" maskcontentunits="userSpaceOnUse" maskunits="objectBoundingBox"  width="24" x="0" y="0">
						<use xlink:href="#path-1"></use>
					</mask>
				</defs>
				<g fill="none" fill-rule="evenodd" id="Symbols" stroke="none" stroke-width="1">
					<g fill="transparent" id="arrow-outline" stroke="#1E1E1E" stroke-width="6">
						<use id="arrow" mask="url(#mask-2)" transform="translate(14.000000, 14.000000) rotate(-90.000000) translate(-14.000000, -14.000000)" xlink:href="#path-1"></use>
					</g>
				</g></svg>
			</div>
			<div class="pillar"></div>
		</div>
		<?php snippet("menu") ?>
	</div>
</div>
